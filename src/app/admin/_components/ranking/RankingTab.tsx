...
export default function RankingTab({ allGames }: RankingTabProps) {
  ...
  // Dynamically get available years from allGames
  const availableYears = useMemo(() => {
    const years = allGames.map((g) => new Date(g.releaseDate).getFullYear());
  ...
  }, [allGames, platform, activeGenre, period, selectedYear, selectedMonth, sortBy]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-white">Visualização do Ranking ({filteredAndSortedGames.length})</h1>
          <p className="text-gray-500 text-sm">
            Esta é uma visualização em tempo real do ranking de jogos público.
            Para editar as notas ou informações de um jogo, acesse a aba de <span className="text-purple-400 font-semibold">Reviews</span>.
          </p>
        </div>
      </div>

      {/* Glassmorphic Filter Box */}
      <div className="bg-[#0f0f18]/60 backdrop-blur-md border border-white/5 rounded-2xl p-6 space-y-6">
      ...
      {podiumGames.length > 0 && (
        <div className="grid grid-cols-3 gap-4 items-end mb-8 max-w-2xl mx-auto bg-[#0a0a12]/30 border border-white/5 p-6 rounded-3xl backdrop-blur-sm">
