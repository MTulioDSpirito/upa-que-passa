...
- `/admin/sugestoes` — review UI (`SugestoesClient.tsx`). Approve/reject calls `/api/admin/entregas/aprovar|rejeitar`, which update the row's status (+ reviewer id, + rejection reason). **Approval creates production content**: `AprovarSugestaoService` runs the matching publisher in `src/core/publishers.ts` (`NewsPublisher` → `appendAdminNews` → `NewsArticle`; `ReviewPublisher`; `LaunchPublisher`), so approving a NOTICIA publishes the article to the live DB — no manual mock editing.
- `POST /api/admin/entregas` creates a PENDING `SugestaoAgente` (auth + zod + dedupe by slug+tipo). This is how agents feed the **live** review queue: `registrar-sugestao.ts` posts here when `UPA_ADMIN_EMAIL`/`UPA_ADMIN_PASSWORD` (and optional `UPA_PROD_URL`) are set; without those env vars it falls back to inserting into the **local** DB (dev only, won't show on the live site). Full loop: agent generates → suggestion lands in prod `/admin/sugestoes` → you approve the ones you want → publisher makes it live.
